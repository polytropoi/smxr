...
unity_router.get('/scene/:_id/:platform/:version', function (req, res) { ////called from unity app context - TODO lock down w/ checkAppID, requiredAuthentication (i.e. )
    ...
    (async () => {
        try {
            ...
            let query = { "short_id" : req.params._id};
            if (ObjectId.isValid(req.params._id)) {
                const s_id = ObjectId.createFromHexString(req.params._id);
                query = {"_id": s_id};
            } 
            ...
            if (sceneData.scenePictures != null && sceneData.scenePictures.length > 0) {
                ...
                // async.each (sceneResponse.sceneWebLinks, function (objID, callbackz) { //nested async-ery!
                    for (let i = 0; i < sceneResponse.sceneWebLinks; i++) {
                        if (ObjectId.isValid(sceneResponse.sceneWebLinks[i])) {
                            
                            // db.weblinks.findOne({'_id': ObjectID(objID)}, function (err, weblink) {
                        const wlquery = {'_id': ObjectId.createFromHexString(sceneResponse.sceneWebLinks[i])};
                        let weblink = await RunDataQuery("weblinks", "findOne", wlquery);             
                ...
                if (sceneResponse.sceneVideoGroups != null && sceneResponse.sceneVideoGroups.length > 0) { //TODO do all the other group types...
                    let vgID = sceneResponse.sceneVideoGroups[0];
                    let oo_id = ObjectId.createFromHexString(vgID);
                    const groupquery = {"_id": oo_id};
                    ...
                    const vidquery = {"_id" : {$in : ids}};
                    let videos = await RunDataQuery("video_items", "find", vidquery);

                    for (let video in videos) { //this is stupid, should use hls..
                        ...
                        console.log("setting video url " + video.url);
                    }
                    vidGroup.videos = videos;
                    requestedVideoGroups.push(vidGroup);
                }

                if (sceneResponse.sceneUseEnvironment) { //i.e. asset bundle scene
                ...
                let audio_items = await RunDataQuery("audio_items", "find", audioquery);
...
//                            audio_items.URLmp3 = urlMp3; //jack in teh signed urls into the object array
                ...
                const picture_items = await RunDataQuery("image_items", "find", picquery);
                ...
                if (sceneResponse.scenePostcards != null && sceneResponse.scenePostcards.length > 0) {
                    ...
                    const postcardID = sceneResponse.scenePostcards[0]; //whatever, just one for now
                        const p_id = ObjectId.createFromHexString(postcardID);
                        const picturequery = {"_id": p_id};
                        const picture_item = await RunDataQuery("image_items", "findOne", picturequery);
                ...
                sceneResponse.postcards = postcards;
                ...
                if ((sceneResponse.userName == null || sceneResponse.userName.length < 1) && (sceneResponse.user_id != null)) {
                    ...
                    const uquery = {"_id": oo_id};
                    const user = await RunDataQuery("users", "findOne", uquery);
                    sceneResponse.userName = user.userName;
                }

                res.json(sceneResponse);
        } catch (e) {
            console.log("error returning scene data... " +e);
            res.send("error returning scene data... " +e);
        }
...
unity_router.get("/unity/:id", function (req, res) { //redirect to unity
    ...
    (async () => {
        try {
            ...
            console.log("error redirecting to unity " + e);
            res.send("error redirecting to unity " + e);
        }
