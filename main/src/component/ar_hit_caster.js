...
function isMobile() {
  ...
  AFRAME.registerComponent("ar_hit_caster", {
    ...
    init: function () {
      ...
      // this.el.sceneEl.setAttribute('webxr', {optionalFeatures: 'dom-overlay, hit-test, local-floor, light-estimation, anchors'});
      let settings = {};
      ...
      this.el.sceneEl.addEventListener("enter-vr", function (event) {
        ...
        console.log(
          "enter-vr w/ aframe checkHeadsetConnected " +
        ...
        if (
          settings &&
          (settings.useXrRoomPhysics || settings.useRealWorldMeshing)
        ) {
          if (
            AFRAME.utils.device.checkHeadsetConnected() &&
            !AFRAME.utils.device.isMobile() &&
            ...
            //isTouchDevice is true on Quest3!
            console.log("tryna do mixed!");
  
            self.arHitTestEnabled = true;
            ...
            self.messageEl.textContent =
              "Mixed Reality not supported on this device!";
            console.log("Mixed Reality not supported on this device!");
            this.el.sceneEl.removeAttribute("xr_room_physics");
            this.el.sceneEl.removeAttribute("real-world-meshing");
            // }
          }
        } else {
          //just in case...
        ...
        self.el.sceneEl.addEventListener(
          "ar-hit-test-start",
          function () {
            self.messageEl.textContent = `Scanning environment, finding surface.`;
          },
          { once: true }
        );
        ...
        self.el.sceneEl.addEventListener(
          ...
          function () {
            self.messageEl.textContent = `Select the location to place content`;
          },
          { once: true }
        );
        ...
        self.el.sceneEl.addEventListener(
          ...
          function () {
            ...
            self.messageEl.textContent = "Successful placement on hit test surface!";
          },
          { once: true }
        );
  
        session.addEventListener("selectstart", function () {
          console.log("session select! " + self.lockTargets + " " + self.arHitTestEnabled);
          ...
          var scaleMod = 0.02 + distance * 0.2;
          
          const arLockButton = document.getElementById("arLockButton");
          if (arLockButton) {
            arLockButton.style.visibility = "visible";
          }
          ...
          if (!self.lockTargets && self.arHitTestEnabled) {
            ...
            self.messageEl.textContent =
              "surface found at position " + JSON.stringify(position);
            ...
            if (self.el.components.raycaster) {
              //don't need this one in vr, bc right hand laser?
              ...
              for (const el of els) {
                ...
                if (elVisible) {
                  const details =
                    self.el.components.raycaster.getIntersection(el);
    ...
    disableARButtons: function () {
      let arOverlay = document.getElementById("dom-overlay");
      if (arOverlay) {
        console.log("gotsa overlay makin invisible");
        arOverlay.style.visibility = "hidden";
      }
    },
    enableARButtons: function () {
      let arOverlay = document.getElementById("dom-overlay");
      if (arOverlay) {
        console.log("gotsa overlay makin visible");
        arOverlay.style.visibility = "visible";
      }
      
      const toggleLockButton = document.getElementById('arLockButton');
      if (toggleLockButton) {
        console.log("gotsa toggleLockButton")
        toggleLockButton.addEventListener('click', () => {
          console.log("tryna click togglelockbutton");
          ToggleLockTargetElements();
        });
      }
      const arScaleUpButton = document.getElementById('arScaleUpButton');
      if (arScaleUpButton) {
        console.log("gotsa scaleUpButton");
        arScaleUpButton.addEventListener('click', () => {
          console.log("gotsa scaleup click!");
            this.scaleTargetElements("up");
        });
      }
      const arScaleDownButton = document.getElementById('arScaleDownButton');
      if (arScaleDownButton) {
        console.log("gotsa scaleDownButton");
        arScaleDownButton.addEventListener('click', () => {
          console.log("gotsa scaledown click!");  
          this.scaleTargetElements("down");
        });
      }
      const arRotRightButton = document.getElementById('arRotRightButton');
      if (arRotRightButton) {
  
        arRotRightButton.addEventListener('click', () => {
          console.log("gotsa rotright click!");  
          this.rotateTargetElements("right");
        });
      }
      const arRotLeftButton = document.getElementById('arRotLeftButton');
      if (arRotLeftButton) {
        
        arRotLeftButton.addEventListener('click', () => {
          console.log("gotsa rotleft click!");  
          this.rotateTargetElements("left");
        });
      }
    },
    spawnElement: function (position, rotation, scaleMod, spawnableEl) {
      ...
      if (localMarker) {
        ...
        spawnedEl.setAttribute("local_marker", {
        ...
        this.messageEl.textContent =
          "spawning local marker at position " + JSON.stringify(position);
      } else {
        ...
        if (cloudMarker) {
          ...
          var obj = spawnableEl.getObject3D("mesh");
          spawnedEl.setObject3D("mesh", obj.clone());
          spawnedEl.object3D.scale.set(
            scaleMod * data.xscale,
            scaleMod * data.yscale,
            scaleMod * data.zscale
          );
          spawnedEl.classList.add("activeObjexRay");
          sceneEl.appendChild(spawnedEl);
          spawnedEl.setAttribute("position", position); //?
          const randomRot = getRandomInt(180);
          spawnedEl.setAttribute("rotation", {
            x: rotation.x,
            y: rotation.y,
            z: randomRot,
          });
          ...
          spawnedEl.object3D.visible = true;
          this.messageEl.textContent =
            "spawning marker at position " + JSON.stringify(position);
        } else {
          ...
          if (modModel) {
            ...
            spawnedEl.object3D.visible = true;
            this.messageEl.textContent =
              "spawning model at position " + JSON.stringify(position);
          } else {
            ...
            if (modObject) {
              ...
              spawnedEl.object3D.visible = true;
              this.messageEl.textContent =
                "spawning object at position " + JSON.stringify(position);
            }
          }
        }
      }
      ...
      // if (targetEl) {
        ...
        if (dir == "up") {
          targetEl.setAttribute("scale", {
            x: targetScale.x + 0.05,
            y: targetScale.x + 0.05,
            z: targetScale.x + 0.05,
          });
        } else {
          if (targetScale.x > 0.1) {
            targetEl.setAttribute("scale", {
              x: targetScale.x - 0.05,
              y: targetScale.x - 0.05,
              z: targetScale.x - 0.05,
            ...
            if (targetScale.x > 0.001) {
              targetEl.setAttribute("scale", {
                x: targetScale.x - 0.001,
                y: targetScale.x - 0.001,
                z: targetScale.x - 0.001,
              });
            } else {
              if (targetScale.x > 0.0001) {
                targetEl.setAttribute("scale", {
                  x: targetScale.x - 0.0001,
                  y: targetScale.x - 0.0001,
                  z: targetScale.x - 0.0001,
                });
              }
            }
          }
    ...
    rotateTargetElements: function (dir) {
      var targetEl = this.el;
        ...
        if (dir == "right") {
          targetEl.setAttribute("rotation", {
            x: targetRot.x,
            y: targetRot.y + 10,
            z: targetRot.z,
          });
    ...
    toggleLockElements: function () {
      console.log("tryna toggle lock elements")
      if (this.arHitTestEnabled) {
        ...
        if (targetEl) {
          if (this.lockTargets) {
            ...
            this.messageEl.textContent = "hit test disabled, target locked, raycaster enabled";
    ...
    tick: function () {
      ...
      if (
        ...
        !this.lockTargets &&
        this.arHitTestEnabled
      ) {
        if (!this.viewerSpace) {
          console.log("no viewerSpace");
          return;
        }
        ...
        if (this.xrHitTestSource && xrViewerPose) {
          ...
          if (hitTestResults.length > 0) {
            ...
            // console.log("gotsa hittest " + JSON.stringify(position) );
  ...
  AFRAME.registerComponent("pinch_fu", {
    ...
    spawn: function (evt) {
      ...
      spawnedEl.setAttribute("geometry", {
        primitive: "box",
        width: "1",
        height: "1",
      });
  ...
  function ToggleLockTargetElements() {
    let arHitCasterEl = document.getElementById("ar_parent");
    if (arHitCasterEl) {
    ...
    tick: function () {
      ...
      for (var i = 0; i < handEls.length; i++) {
        ...
        if (distance < this.data.pressDistance) {
          ...
          this.pressed = true;
          console.log(
            this.el.id + " pressable pressed with finger distance " + distance
          );
          return;
        }
      }
