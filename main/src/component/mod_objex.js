...
AFRAME.registerComponent('mod_objex', {
    schema: {
      ...
      init: function () {
        ...
        // console.log("objxe location datas" + JSON.stringify(this.data.jsonObjectData));
        ...
        let that = this;
        this.posRotReader = null;
        const player = document.getElementById("player");
        if (player) {
          this.posRotReader = player.components.get_pos_rot; 
        }
      ...
      shootObject: function (objectID) {
        ...
        if (this.posRotReader) {
          this.playerPosRot = this.posRotReader.returnPosRot(); 
          ...
          this.objEl.id = "obj" + this.objectData._id + "_" + this.locData.timestamp;
          //this.el.sceneEl.append(this.objEl);
          if (settings && settings.useArParent) {                
            let ar_parent = document.getElementById("ar_parent");
            if (ar_parent) {
              ar_parent.appendChild(this.objEl);
...
AFRAME.registerComponent('mod_object', {
      ...
      if (this.data.objectData.actions != undefined && this.data.objectData.actions.length > 0) {
        ...
        for (let a = 0; a < this.data.objectData.actions.length; a++) {
          if (this.data.objectData.actions[a].objectID && this.data.objectData.actions[a].objectID.length > 8 ) {
            FetchSceneInventoryObject(this.data.objectData.actions[a].objectID); //umm, should check if this is already fetched
          }
    ...
    this.el.addEventListener('model-loaded', () => {
        ...
        if (this.loadAction != null) {
          ...
          if (obj) {
            ...
            if (hasCallout) {
              ...
              // } else {
              //  this.el.sceneEl.appendChild(bubble); //or else put at top
              // }
      ...
      this.el.addEventListener('mouseenter', (evt) => {
        ...
        evt.preventDefault();
        if (!this.data.isEquipped) {
          if (this.posRotReader != null) {
            this.playerPosRot = this.posRotReader.returnPosRot(); 
            window.playerPosition = this.playerPosRot.pos; 
          } else {
            this.posRotReader = document.getElementById("player").components.get_pos_rot; 
            this.playerPosRot = this.posRotReader.returnPosRot(); 
            window.playerPosition = this.playerPosRot.pos; 
          }
      ...
      this.el.addEventListener('mouseleave', (e) => { 
        ...
        if (!this.data.isEquipped) {
          ...
          this.bubble = this.el.sceneEl.querySelector('.bubble');
          if (this.bubble) {
            this.bubble.setAttribute('visible', false);
          }
        }
        ...
        if (this.killAction.actionResult.toLowerCase() == "spawn") {
          if (!this.isTriggered) {
            this.isTriggered = true;
              ...
              if (objectData == null) {
                objectData = this.objexEl.components.mod_objex.returnObjectData(this.killAction.objectID); //dbl chkn
              }
              if (objectData != null) {
